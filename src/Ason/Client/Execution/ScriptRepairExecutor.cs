...
internal sealed class ScriptRepairExecutor : IScriptRepairExecutor {
    public async Task<ExecOutcome> ExecuteWithRepairsAsync(
        ...
        for (int attempt = 0; attempt <= maxAttempts; attempt++) {
            ...
            log(LogLevel.Debug, $"Script attempt {attempt + 1}:\n{userScript}", null);
            var validationError = validator.Validate(userScript);
            if (validationError is not null) {
                lastError = validationError; lastScript = userScript; log(LogLevel.Warning, $"Validation failed: {validationError}", null);
                if (attempt == maxAttempts) return new ExecOutcome(false, null, validationError, userScript, attempt + 1);
                continue;
            }
            ...
            try {
                ...
                lastError = ex.Message; lastScript = userScript; log(LogLevel.Error, "Execution error", ex);
