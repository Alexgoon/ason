...
"""
        ...
        3. Avoid generating double awaits. For example 'await (await someOperator.GetAnotherOperatorAsync()).SomeMethod()' is forbidden - treat this as a compilation error. Split them into two lines.
        4. Don't chain await calls with other methods. For example, await someOperator.GetAnotherOperatorAsync().SomeMethod() is forbidden. Split them into two lines.
        5. Methods must only be called on the class or object where they are defined:
        ...
        6. Absolutely forbidden: creating operator instances with `new`:
        ...
        7. If the task cannot be completed based on the available API and standard C# functions, output a single plain English sentence starting with the word "Cannot" explaining briefly (no code, no XML tags). Do NOT output any code in that case.
    ...
    public const string AnswerRouterTemplate =
        ...
        - Do NOT repeat prior conversation verbatim; summarize succinctly.
        """;
