...
public class AsonClient : IChatClient {
    ...
    internal AsonClient(
        ...
        _runner = new RunnerClient(rootOperator.OperatorInstances, SynchronizationContext.Current) { Mode = _options.RunnerMode };
        SetupCommonLogging();
    ...
    (RouteKind route, string payload) DecideRouteAdvance(string userTask, bool skipAnswer, ChatCompletionAgent? answerAgent, ChatHistoryAgentThread thread, Action<LogLevel, string, Exception?> log, CancellationToken ct) {
        ...
        try {
            _ = Task.Run(async () => {
                ...
                await foreach (var item in answerAgent.InvokeAsync(messages, thread: thread, options: null, cancellationToken: ct)) {
                    var part = item.Message?.Content; if (!string.IsNullOrWhiteSpace(part)) sb.Append(part);
                }
                tcs.TrySetResult(sb.ToString());
            }, ct);
            var decisionRaw = tcs.Task.GetAwaiter().GetResult();
    ...
    async Task<OrchestrationResult> ExecuteScriptRouteNonStreamingAsync(string task, ChatHistoryAgentThread thread, bool skipExplainer, CancellationToken cancellationToken) {
        ...
        string explanation = await _resultExplainer.ExplainAsync(effectiveTask, outcome.RawResult!, _explainerAgent!, (lvl, msg, ex) => OnLog(lvl, msg, ex), cancellationToken);
    ...
    async Task<OrchestrationResult> SendDetailedAsync(ChatHistoryAgentThread thread, CancellationToken cancellationToken = default) {
        ...
        var (route, payload) = DecideRouteAdvance(userTask, skipAnswer, _answerAgent, thread, (lvl, msg, ex) => OnLog(lvl, msg, ex), cancellationToken);
        ...
        return await ExecuteScriptRouteNonStreamingAsync(effectiveTask, thread, skipExplainer, cancellationToken);
    }
    ...
    public async Task<string> ExecuteScriptDirectAsync(string script, bool validate = true) {
        ...
        try {
            var execResult = await _runner.ExecuteAsync(code);
    ...
    async IAsyncEnumerable<ChatResponseUpdate> InternalStreaming(IEnumerable<ChatMessage> messages, [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken) {
        ...
        if (skipAnswer) { OnLog(LogLevel.Information, "Skipping AnswerAgent; routing directly to ScriptAgent."); _consolidatedUserTask = userTask; proceedToScript = true; }
            ...
            var messagesSk = new[] { new ChatMessageContent(AuthorRole.User, userTask) };
            var sb = new StringBuilder(); // full raw answer agent output (may include script + task block)
            bool bufferingPossibleScript = true; // still determining if output is the literal word 'script'
            bool collectingTaskBlock = false; // after detecting script, we collect <task> ... </task> silently
            await foreach (var item in _answerAgent!.InvokeStreamingAsync(thread: thread, options: null, cancellationToken)) {
                var part = item.Message?.Content;
                if (string.IsNullOrWhiteSpace(part)) continue;

                // Always append to master buffer (internal use)
                sb.Append(part);
                ...
                if (bufferingPossibleScript) {
                    ...
                    if (current.Equals("script", StringComparison.OrdinalIgnoreCase)) {
                        ...
                        collectingTaskBlock = true; // expect optional <task> block
                        continue; // do not emit 'script' token to user
                    }
                    if ("script".StartsWith(current, StringComparison.OrdinalIgnoreCase)) {
                        // Still potentially the word 'script'; wait for more tokens
                        continue;
                    }
                    // Not the routing keyword -> treat entire accumulated buffer as an answer and stream once
                    yield return new ChatResponseUpdate(ChatRole.Assistant, current);
                    bufferingPossibleScript = false; // normal answer mode
                }
                else if (collectingTaskBlock) {
                    // We are in script route; suppress streaming until we close </task> (or some safety condition)
                    if (sb.ToString().IndexOf("</task>", StringComparison.OrdinalIgnoreCase) >= 0) {
                        // Collected complete task block; stop reading further from answer agent
                        break;
                    }
                    // Keep collecting silently
                    continue;
                }
                else {
                    // Regular answer path (already determined not a script route) -> stream incremental parts
            ...
            if (proceedToScript) {
                ...
                if (string.IsNullOrWhiteSpace(full)) { proceedToScript = true; _consolidatedUserTask = userTask; }
                    ...
                    yield break; // answer path
                }
            }
        }
        if (proceedToScript) {
            var effectiveTask = _consolidatedUserTask ?? userTask;
            await foreach (var u in StreamScriptRouteAsync(effectiveTask, thread, skipExplainer, cancellationToken)) yield return u;
        }
    }
