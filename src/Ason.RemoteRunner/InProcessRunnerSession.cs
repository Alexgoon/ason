...
internal sealed class InProcessRunnerSession : ScriptRunnerSession {
    ...
    private sealed class InProcBridge : AsonHostInterop.IHostBridge {
        ...
        public Task<T?> InvokeAsync<T>(string target, string method, object?[]? args = null, string? handleId = null) => Task.FromResult<T?>(default);
        public Task<T?> InvokeMcpAsync<T>(string server, string tool, IDictionary<string, object?>? arguments = null) => Task.FromResult<T?>(default);
    }
}
