...
static class FakeDataService
{
    static readonly List<Employee> _employees = new();
    static readonly List<MailItem> _mailItems = new();
    static readonly List<Appointment> _appointments = new();
    static readonly string[] _productNames = ["CRM Subscription","Implementation Package","Training Session","Premium Support","Analytics Add-on"];    
    ...
    static void Seed()
    {
        if (_employees.Count > 0) return; // already seeded
        ...
        // Create sample mail items (customer emails)
        ...
        // Create appointments (meetings, demos, internal syncs)
        string[] apptTitles = [
            "Discovery Call - ACME",
            "Product Demo - Globex",
        ...
        for (int i = 0; i < 20; i++)
        {
            var dayOffset = -10 + i; // distribute across a ~20 day window
    ...
    public static Task<List<Employee>> GetEmployeesAsync() => SimulateLatency(_employees);
    public static Task<List<MailItem>> GetMailItemsAsync() => SimulateLatency(_mailItems);
    public static Task<List<Appointment>> GetAppointmentsAsync() => SimulateLatency(_appointments);

    static async Task<List<T>> SimulateLatency<T>(List<T> source)
    {
        await Task.Delay(Random.Shared.Next(1000, 1500));
        // Return shallow copy to mimic detached data / prevent external mutation of internal list
