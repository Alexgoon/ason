...
public class LoggingAndEventsTests {
    ...
    private AsonClient CreateClient(IChatCompletionService svc, IScriptRepairExecutor? repair = null) {
        ...
        var client = new AsonClient(svc, root, snapshot, new AsonClientOptions { SkipAnswerAgent = false, SkipExplainerAgent = true, RunnerMode = ExecutionMode.InProcess }, repair, new KeywordScriptValidator(Array.Empty<string>()), new ResultExplainer());
        return client;
    }
