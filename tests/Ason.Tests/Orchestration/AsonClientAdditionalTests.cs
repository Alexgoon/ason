...
public class AsonClientAdditionalTests {
    ...
    private static AsonClient CreateClient(IChatCompletionService answerSvc, IScriptRepairExecutor repair, AsonClientOptions? opts = null) {
        ...
        var client = new AsonClient(answerSvc, root, Snapshot, new AsonClientOptions {
            ...
            RunnerMode = ExecutionMode.InProcess
    ...
    public async Task Concurrent_Executions_IsolatedClients() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            var root = new RootOperator(new object());
            var client = new AsonClient(chat, root, Snapshot, new AsonClientOptions { SkipAnswerAgent = true, SkipExplainerAgent = false, RunnerMode = ExecutionMode.InProcess }, null, new KeywordScriptValidator(Array.Empty<string>()), null);
    ...
    public async Task ProxyRegeneration_Unchanged_When_No_New_Operators() {
        ...
        var root = new RootOperator(new object());
        var client = new AsonClient(chat, root, Snapshot, new AsonClientOptions { SkipAnswerAgent = true, SkipExplainerAgent = false, RunnerMode = ExecutionMode.InProcess });
    ...
    public async Task LargeStreamingScriptAssembly() {
        ...
        var root = new RootOperator(new object());
        var client = new AsonClient(streamer, root, Snapshot, new AsonClientOptions { SkipAnswerAgent = false, SkipExplainerAgent = true, RunnerMode = ExecutionMode.InProcess });
